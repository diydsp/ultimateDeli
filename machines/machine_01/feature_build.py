...
with open( filename_input ) as csvfile:
    ...
    with open( filename_output, 'w' ) as data_out:
        ...
        for idx in range( indic_arr_len - 1 ):
            ...
            open_today  = indicator_array[     idx ][ 0 ]
            high_today  = indicator_array[     idx ][ 1 ]
            low_today   = indicator_array[     idx ][ 2 ]
            close_today = indicator_array[     idx ][ 3 ]
            open_tmw    = indicator_array[ 1 + idx ][ 0 ]
            high_tmw    = indicator_array[ 1 + idx ][ 1 ]
            low_tmw     = indicator_array[ 1 + idx ][ 2 ]
            close_tmw   = indicator_array[ 1 + idx ][ 3 ]
            
            ratio = close_tmw / close_today;
            #ratio = open_tmw / open_today;
            #ratio = high_tmw / open_tmw;
            #ratio = close_tmw / open_tmw;
            #ratio = high_tmw / low_tmw;
